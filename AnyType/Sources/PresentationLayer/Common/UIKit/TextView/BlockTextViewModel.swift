...
class BlockTextViewModel {
    ...
    init(blockViewModel: BaseBlockViewModel) {
        ...
        self.coordinator = BlockTextViewCoordinator(menuItemsBuilder: BlockActionsBuilder(restrictions: restrictions),
...
// MARK: View
extension BlockTextViewModel {
    func createView() -> BlockTextView {
        BlockTextView().configured(self)
    }

    func createView(_ options: BlockTextView.Options) -> BlockTextView {
        BlockTextView().configured(options).configured(self)
    }
}
