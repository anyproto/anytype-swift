...
class PageIconUIKitView: UIView {
    ...
    weak var viewModel: PageIconViewModel? {
        didSet {
            self.viewModel?.$toViewEmoji.reciveOnMain().sink(receiveValue: { [weak self] (value) in
                self?.text = value
            }).store(in: &self.subscriptions)
            self.viewModel?.$toViewImage.reciveOnMain().sink(receiveValue: { [weak self] (value) in
                self?.image = value
            }).store(in: &self.subscriptions)
        }
    }
