...
public class SearchHelper {
    ...
    public static func sort(
        ...
        sort.type = type
        sort.noCollate = noCollate
        if let emptyPlacement { sort.emptyPlacement = emptyPlacement }
        enrichDateMetadata(&sort, for: relationKey)
        return sort
    }
    ...
    public static func identities(_ identityIds: [String]) -> DataviewFilter {
        var filter = DataviewFilter()
        filter.condition = .in
        filter.value = identityIds.protobufValue
        filter.relationKey = BundledPropertyKey.identity.rawValue
        return filter
    }
    ...
    private static func enrichDateMetadata(_ sort: inout DataviewSort, for relationKey: String) {
        if BundledPropertyKey.dateKeys.map({ $0.rawValue}).contains(relationKey) {
            sort.format = .date
            sort.includeTime = true
        }
    }
