    ...
    private func setupView() {
        rows = .builder {
            if let version = appVersion, version.isNotEmpty {
                createRow(title: Loc.About.appVersion, value: version)
            }
            if let buildNumber = buildNumber, buildNumber.isNotEmpty {
                createRow(title: Loc.About.buildNumber, value: buildNumber)
            }
            if let libraryVersion = libraryVersion, libraryVersion.isNotEmpty {
                createRow(title: Loc.About.library, value: libraryVersion)
            }
            if let analyticsId = analyticsId, analyticsId.isNotEmpty {
                createRow(title: Loc.About.analyticsId, value: analyticsId)
            }
        }
    }
