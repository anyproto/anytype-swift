...
struct PropertyCalendarCoordinatorView: View {
    ...
    @State private var model: PropertyCalendarCoordinatorViewModel

    init(date: Date?, configuration: PropertyModuleConfiguration) {
        _model = State(initialValue: PropertyCalendarCoordinatorViewModel(date: date, configuration: configuration))
    }
