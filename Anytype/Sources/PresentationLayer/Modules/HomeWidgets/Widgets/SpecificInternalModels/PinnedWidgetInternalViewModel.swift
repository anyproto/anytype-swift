...
@MainActor
final class PinnedWidgetInternalViewModel: ObservableObject, WidgetInternalViewModelProtocol {
    ...
    @Published private var name: String = Loc.pinned
    ...
    func screenData() -> ScreenData? {
        return .editor(.pinned(homeObjectId: homeObjectId, spaceId: spaceId))
    }
    ...
    func onCreateObjectTap() {
        Task {
            ...
            try await objectActionsService.setPin(objectIds: [details.id], true)
    ...
    private func updateSubscription(widgetInfo: BlockWidgetInfo) {
        ...
        pinnedSubscriptionService.startSubscription(
