...
final class SpaceTypeChangeViewModel {
    ...
    private var expectedSpaceType: SpaceType?
    ...
    func startSubscriptions() async {
        for await spaceType in workspaceStorage.spaceViewPublisher(spaceId: data.spaceId).map(\.spaceType).removeDuplicates().values {
            chatIsSelected = spaceType == .chat
            dataIsSelected = spaceType == .regular
        }
    }
    ...
    func onTapData() async throws {
        ...
        expectedSpaceType = .regular
        showAlert = true
    }

    func onChange() async throws {
        guard let expectedSpaceType else { return }
        try await workspaceService.workspaceSetDetails(spaceId: data.spaceId, details: [.spaceType(expectedSpaceType)])
    }
}
