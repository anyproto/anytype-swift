...
struct OldSpaceHubView: View {
    ...
    private func spaceCard(_ space: ParticipantSpaceViewDataWithPreview) -> some View {
        OldSpaceCard(
            space: space,
            wallpeper: model.wallpapers[space.space.spaceView.targetSpaceId] ?? .default,
            draggedSpace: $draggedSpace,
            onTap: {
                model.onSpaceTap(spaceId: space.space.spaceView.targetSpaceId, presentation: .widgets)
            },
            onTapCopy: {
                model.copySpaceInfo(spaceView: space.space.spaceView)
            },
            onTapPin: {
            ...
            onTapLeave: {
                model.leaveSpace(spaceId: space.space.spaceView.targetSpaceId)
            },
            onTapDelete: {
                try await model.deleteSpace(spaceId: space.space.spaceView.targetSpaceId)
            }
        )
        .equatable()
    }
