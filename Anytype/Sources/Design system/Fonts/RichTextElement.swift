import SwiftUI
 ...
 // Source: https://www.avanderlee.com/swiftui/text-weight-combinations/

 @available(iOS, deprecated: 15, message: "Use AttributedString or Text view for markdown format")
 struct RichTextElement: Identifiable {
 ...
 extension String {
     ...
     func parseRichTextElements() -> [RichTextElement] {
         ...
         self[firstRange].components(separatedBy: " ").forEach { (word) in
             ...
             elements.append(RichTextElement(content: String(word), isBold: false))
         }
         ...
         for (index, matchingRange) in matchingRanges.enumerated() {
             ...
             elements.append(RichTextElement(content: matchContent, isBold: true))
