...
class ImageLoaderObject: ObservableObject {
    private let imageService: ImageService = .init()
        ...
        return self.imageService.fetchImageAndIgnoreError(url: self.url)
            .handleEvents(receiveSubscription: { [weak self] _ in self?.onStart()},
                          receiveOutput: { value in
                            ImageCache.shared.setImage(url: url, image: value)
//                                self.onOutput(value)
                          },
    ...
    //        self.loadImageSubscription = self.imageService.fetchImage(url: self.path).receiveOnMain().sink(receiveValue: { [weak self] (value) in
    //            self?.image = value
    //        })
    //    }
}
