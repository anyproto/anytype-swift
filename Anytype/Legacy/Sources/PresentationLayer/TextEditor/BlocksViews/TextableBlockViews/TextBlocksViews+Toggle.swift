    ...
    class BlockViewModel: TextBlocksViews.Base.BlockViewModel {
        ...
        override init(_ block: BlockModel) {
            ...
            self.$toggled.sink { [weak self] (value) in
                ...
                me.didReceiveAction(generalAction: .buttonView(.toggle(.toggled(value))))
            }.store(in: &self.subscriptions)
            
            self.$insertFirst.sink { [weak self] (value) in
                guard let me = self else { return }
                me.didReceiveAction(generalAction: .buttonView(.toggle(.insertFirst(value))))
            }.store(in: &self.subscriptions)
        }
        ...
        override func makeUIView() -> UIView {
            ...
            return UIKitView()
                ...
                .configured(hasChildren: { [weak self] in self?.getRealBlock().blocks.isEmpty == false } )
